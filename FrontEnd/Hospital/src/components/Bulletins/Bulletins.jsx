...
export default function Bulletins({
  ...
  // Mark read (socket → http fallback)
  ...
  const markAllRead = () => {
    ...
    setHistory((prev) =>
      prev.map((m) =>
        m.readAt === null ? { ...m, readAt: new Date().toISOString() } : m
      )
    );
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className="flex-1 space-y-2 overflow-y-auto pr-1 scrollbar-none"
        ...
        <AnimatePresence initial={false}>
          {bulletins.length === 0 ? (
            ...
            >
              ...
              <p className="text-[11px] text-gray-400">No active announcements</p>
            </motion.div>
          ) : (
            bulletins.slice(0, maxItems).map((b) => {
              const isLong = (b?.content?.length || 0) > 120;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 flex-1 min-w-0">
                    <p
                      className="
                        text-[12px] text-gray-700 dark:text-gray-300 leading-snug
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {isLong && (
                        <span className="text-[9px] font-bold text-rose-500/70 uppercase tracking-wider">
                          Read more
                        </span>
                      )}
                    </div>
