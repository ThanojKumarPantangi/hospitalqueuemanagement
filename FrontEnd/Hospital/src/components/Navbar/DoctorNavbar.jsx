...
const DoctorNavbar = ({ activePage = 'Home' }) => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const navigate = useNavigate();
  const { user } = useAuth();

  // Updated links specifically for the Doctor role
  const navLinks = [
    { name: 'Home', href: '/doctor/dashboard' },
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 border-2 border-white dark:border-gray-900 shadow-sm overflow-hidden">
                {/* Changed seed to represent a doctor profile */}
        ...
        <div className="px-4 pt-2 pb-6 space-y-2 bg-white dark:bg-gray-950">
          ...
          <div className="pt-4 mt-4 border-t border-gray-100 dark:border-gray-800 space-y-4">
             <div className="flex items-center gap-3 px-4">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-700 dark:text-gray-300 leading-none">{user?.name?.toUpperCase() || "DOCTOR"}</p>
                  <p className="text-[10px] text-teal-500 font-bold">{user?.role || "SPECIALIST"}</p>
                </div>
             </div>
