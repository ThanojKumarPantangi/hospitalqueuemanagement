...
const PatientProfile = () => {
  ...
  const markDirtyIfNeeded = (newUser, newProfile) => {
    ...
    const dirty =
      !deepEqual(newUser, backupUser) || !deepEqual(newProfile, backupProfile);
    setIsDirty(dirty);
  };
                  ...
                  <div className="flex-1 w-full min-w-0 mb-2">
                    {isEditing ? (
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-white/80 uppercase tracking-widest shadow-sm">
                          Display Name
                        </label>
                        <input
                          name="name"
                      ...
                      {!isEditing && (
                        <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 font-medium">
                          ...
                          <button
                            ...
                            className="ml-1 inline-flex items-center justify-center p-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-white transition-all active:scale-95"
                            title="Show QR for Admin Scan"
                          >
                            <QrCode size={16} />
                          </button>
                        </div>
          ...
          {showQr && (
            ...
            >
              <motion.div
                ...
                className="w-full max-w-sm rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl p-5"
              >
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                  Show this QR to the admin for quick token booking.
                </p>

                <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 p-4 flex items-center justify-center min-h-[240px]">
                  {qrLoading ? (
                    <span className="text-sm text-gray-400">
                      Generating QR...
                    </span>
                  ) : qrError ? (
                    <span className="text-sm text-red-500">{qrError}</span>
                  ) : qrImage ? (
                    <img
                      src={qrImage}
                      alt="Patient QR"
...
const StatBadge = ({ label, value, color, icon: Icon }) => {
  ...
  return (
    <div
      className={`px-4 py-2 rounded-xl border flex flex-col items-start min-w-[100px] ${colors[color]}`}
    >
    ...
    >
      <span
        className={`transition-colors ${
          isEditing ? "text-gray-400 group-focus-within:text-teal-500" : "text-gray-400"
        }`}
      >
        <Icon size={18} />
      </span>
    ...
    >
      ...
      {isEditing ? (
        <div className="flex-1 relative">
          ...
          <ChevronDown
            size={14}
            className="absolute right-0 top-1/2 -translate-y-1/2 text-gray-400 z-0"
          />
        </div>
      ) : (
