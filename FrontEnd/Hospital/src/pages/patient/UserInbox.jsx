...
// (this variant was missing earlier — keep it explicit)
...
export default function UserInbox() {
  ...
  // We use 1024px (lg) as the breakpoint for "Desktop" split view
  const [isDesktop, setIsDesktop] = useState(true);
  const [isTablet, setIsTablet] = useState(false); // md (>=768) & <1024 is tablet
  ...
  useEffect(() => {
    if ((!isDesktop && !isTablet) && activeThread && isSwiping === false) {
      // On phone when a thread is active, lock body scroll to give a native app feel.
  ...
  const loadThreads = useCallback(async () => {
    try {
      ...
      // Gentle notification only on first load if successful
      if (data.threads?.length > 0) {
        // optional subtle toast removed to keep noise down
      }
  ...
  const deriveClosedInfo = (threadObj, msgs = []) => {
    ...
    // 2) Inspect messages for system close
  ...
  const onTouchStart = (e) => {
    if (isDesktop || isTablet) return; // not on larger screens
    const touch = e.touches && e.touches[0];
    if (!touch) return;
    const startX = touch.clientX;
    touchStartXRef.current = startX;
    // only enable if started near left edge (mimic iOS)
    if (startX <= 28) {
  ...
  const onTouchMove = (e) => {
    ...
    const touch = e.touches && e.touches[0];
    if (!touch) return;
    const dx = Math.max(0, touch.clientX - swipeRef.current.startX); // only positive (right swipe)
    // cap for a nice elastic feel
  ...
  const onTouchEnd = (e) => {
    ...
    if (dx > threshold) {
      // trigger back / close thread and animate
      setSwipeX(500); // push it out quickly
      setTimeout(() => {
        handleBackToInbox();
        setSwipeX(0);
      }, 180);
  ...
  // Determine Chat animation state
  const getChatState = () => {
    if (isDesktop) return "desktop";
    if (isTablet) return "tablet";
  ...
  // For tablet dual-pane, we want both to be visible but adjust widths.
  // We'll compute class additions below to avoid removing original structure.
  ...
  const chatExtraClasses = isTablet
    ? "ml-[360px] lg:ml-0" // ensure spacing on small screens but for tablet provide room
    : "";
      ...
      <div className="relative w-full h-full flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden relative h-full">
          ...
          {/* =========================================================================
              ...
              - Tablet: Visible and tuned width (dual-pane)
          ...
          <motion.div
            ...
            style={{
              // Inline width fallback so framer variants can animate width more reliably
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 pb-4 space-y-2 relative z-0">
              {isLoadingThreads ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 text-slate-400 dark:text-slate-500">
                  ...
                  <p className="text-xs mt-1 opacity-70">Start a new conversation to get help</p>
                </div>
              ) : (
          ...
          {/* =========================================================================
              ...
              - Tablet: Dual-pane tuning (both visible)
              - Mobile: Full screen overlay when thread selected, with swipe-back
             ========================================================================= */}
          <motion.div
            ...
            style={{
              // Apply swipe style only on mobile (phone) when swiping
            ...
            <AnimatePresence mode="wait">
              {!activeThread ? (
                ...
                >
                  ...
                  <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-200 mb-3 text-center">How can we help?</h2>
                  <p className="text-sm max-w-md text-center leading-relaxed opacity-80">
                    Select an existing conversation from the sidebar or start a new ticket to get in touch with our medical support team.
                  </p>
                  <button
                    onClick={() => setIsModalOpen(true)}
                  ...
                  >
                    <PlusCircle size={18} /> Start New Ticket
                  </button>
                </motion.div>
              ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h2 className="text-lg font-bold text-slate-800 dark:text-white leading-tight">Medical Support</h2>
                          {isClosed && (
                          ...
                          {messages.map((msg, index) => {
                            ...
                            return (
                              <motion.div
                                ...
                                animate="visible"
                                layout
                                className={`flex gap-4 w-full mb-1 ${isUser ? "flex-row-reverse" : "flex-row"}`}
                              >
                                {/* Avatar */}
                                <div className={`
                                ...
                                <div className={`flex flex-col max-w-[85%] md:max-w-[70%] ${isUser ? "items-end" : "items-start"}`}>
                                  {/* Sender Name */}
                                  {!isUser && index > 0 && messages[index-1].senderRole === "PATIENT" && (
                                    <span className="text-[10px] text-slate-400 ml-2 mb-1">Support Agent</span>
                                  )}
                                  ...
                                  <div className={`flex items-center gap-1 mt-1.5 px-1 opacity-70 hover:opacity-100 transition-opacity ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                                    <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium flex items-center gap-1">
                                      {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </span>
                                    {isUser && isLast && !msg.__optimistic && (
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="relative px-8 pt-8 pb-6">
                  ...
                  <p className="text-indigo-100 text-sm font-medium opacity-90">Describe your issue and we&apos;ll help you shortly.</p>
                </div>
